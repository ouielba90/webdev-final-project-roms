...
function useLicenseValidation(vendor, seats, licenseKey, purchaseDate, expiryDate, cost) {
  ...
  useEffect(() => {
    ...
    } else if (!textRegex.test(vendor)) {
      newErrors.vendor = "Proveedor inválido: solo letras, números, guiones y espacios intermedios.";
    }
    ...
    } else if (licenseKey.length < 2) {
      newErrors.licenseKey = "La clave de licencia debe tener al menos 2 caracteres.";
    } else if (licenseKey.length > 50) {
      newErrors.licenseKey = "La clave de licencia no puede superar los 50 caracteres.";
    } else if (!textRegex.test(licenseKey)) {
      newErrors.licenseKey = "Clave de licencia inválida: solo letras, números, guiones y espacios intermedios.";
    }
    ...
    if (!expiryDate) {
      ...
      newErrors.compareDates = "La fecha de expiración no puede ser anterior a la fecha de compra.";
    }
